...
| Additional regression/property tests | The transmission-wire lesson: no unit test had ever exercised live transmission until session F. **Traces since 2026-09-13 (PR B, alpha.8): `crates/neuralos-snn/tests/traces/`, 12 regression cases and 1 reference vector, compared by the test gate, regenerated by `cargo run -p neuralos-snn --example trace -- write`; C, D and E prove "behavior unchanged" by that compare.** |
