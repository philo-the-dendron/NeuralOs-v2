# NeuralOS v2 — Research Log

> The full session-by-session record of the ternary-bridge arc
> (2026-08-08 → 2026-08-20), extracted verbatim from `docs/VISION.md`
> at the 2026-08-20 project autopsy (rung R2 of the remediation
> ladder). VISION is the north star again; this file is the history.
>
> Cross-references, in order of authority:
>
> - `ISA.md` — the per-session claims/decisions/verifications ledger
>   (sessions G, H, H2, I live **only** there + `evidence/INDEX.md`;
>   the narrative below ends at the session-G era)
> - `paper/` — the distilled article (Branch B) with its own
>   reproducibility appendix
> - `docs/PAPER_OUTLINE.md`, `docs/PAPER_NOVELTY.md` — paper prep
> - `evidence/INDEX.md` — the raw judge/log evidence index
>
> Recovery point: tags `pre-autopsy-cleanup` (main) and
> `pre-autopsy-cleanup-paper` (paper-draft) hold the pre-cleanup tree.
