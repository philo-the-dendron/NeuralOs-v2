...
| `proofs/` | Rebuildable proof harnesses, one standalone crate per proof (own `[workspace]` table — the repo workspace never sees them). `proofs/qemu-riscv-leg-a/` is the bare-metal riscv64 QEMU gate (evidence: `evidence/qemu-riscv-gate/`). `proofs/no-panic-step/` is the link-time proof that `FixedNetwork::step` has no panic path, under the firmware's profile and one without cross-crate LTO (CI job `firmware`). |
