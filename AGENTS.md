...
| `proofs/` | Rebuildable proof harnesses, one standalone crate per proof (own `[workspace]` table — the repo workspace never sees them). `proofs/qemu-riscv-leg-a/` is the bare-metal riscv64 QEMU gate (evidence: `evidence/qemu-riscv-gate/`). `proofs/no-panic-step/` is the link-time proof that `FixedNetwork::step` has no panic path, under the firmware's profile and one without LTO (CI job `firmware`). |
...
proofs/no-panic-step/build.sh                     # the no-panic proof, same job and loop: its own fmt, then FixedNetwork::step linked under the firmware's profile and under one without LTO, each leg with a canary the linker must refuse by name (the script's header carries the why)
