...
| `proofs/` | Rebuildable proof harnesses, one standalone crate per proof (own `[workspace]` table — the repo workspace never sees them). `proofs/qemu-riscv-leg-a/` is the bare-metal riscv64 QEMU gate and `proofs/qemu-trace-replay/` the frozen traces replayed under QEMU, a release-round gate run by hand (evidence of both: `evidence/qemu-riscv-gate/`). `proofs/no-panic-step/` is the link-time proof that `FixedNetwork::step` has no panic path, under the firmware's profile and one without cross-crate LTO (CI job `firmware`). |
| `firmware/` | Board firmware, one standalone crate per board (own `[workspace]` table, like `proofs/`: esp-hal builds only for the chip target). `firmware/esp32c3/` runs the LIF neuron and a `FixedNetwork` on the ESP32-C3 and replays the host's frozen traces over serial, CI job `firmware`. |
...
PATH="$PWD/.nirenv/bin:$PATH" cargo test -p neuralos-rt --features hdf5  # the hdf5 gate (vendored HDF5; cmake from .nirenv; the count is the CI log's)
...
- `neuralos-snn`: `default = ["std"]`, `std`, `simd` (implies std,
  ...
  every `--workspace` line builds the library with it on),
