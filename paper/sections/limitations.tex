...
(1\,KB$\to$12\,KB corpus), \allowbreak repetition (6$\times\to$\allowbreak
1$\times$), and gain together --- by design of the corpus-infidelity
correction, not by accident. The de-confound arm (true corpus head $\times$ 6 epochs)
is named and unrun.

\paragraph{Sign-dominant drive.} 69.5--69.8\% of dim-steps railed at
the clamp (Sec.~\ref{sec:invivo}); the effective drive is
sign-dominant --- traced by a later probe to a milli/norm-unit domain
mismatch in the scaling application, unreachable by any ceiling the
input path carries. The domain-corrected scaling is the named fix
(report-only arm in the follow-on benchmark); per-dimension
standardization the deeper one; amplitude-graded claims are not
...
\paragraph{The uncalibrated instrument, and what that licenses.}
The adjudication ran against one judge: five prompts, twelve greedy
tokens, one pinned runtime, one $512\times512$ slice of 36 layers,
null families of ten and three draws
(Table~\ref{tab:flipcis} carries the denominators). Within that
scope the instrument itself is uncalibrated: no positive control
establishes that this judge could detect adaptation \emph{content}
if it were present, and no scale ties
$|\Delta\mathrm{margin}|$ to a behavioral magnitude. The null
therefore licenses exactly one claim --- the changes do not exceed
equal-dose random perturbation under this protocol --- and no claim
about the internal magnitude of adaptation effects; that is the
strongest form this negative takes at this scope. Calibration ---
a lesion/graft positive control driven through the same splice
machinery --- precedes any future interpretation and is named as the
first arm of the follow-on readout benchmark. The substrate-side
deferrals (spike-count readout, direct drive, encoder-only
assembly) are mechanism-scoped future work on the substrate's
information flow; none of them bears on the adjudicated question.
