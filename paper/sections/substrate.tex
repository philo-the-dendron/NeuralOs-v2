...
\citep{deshpande2025fullinteger}. The
...
The canary test
(\texttt{recurrent\_current\_is\_never\_}\allowbreak
\texttt{integrated\_in\_}\allowbreak\texttt{step\_bug\_pinned})
...
23{,}958$ (1.63$\times$): \emph{firing tracks the arrangement of the
...
Sec.~\ref{sec:invivo} inherit. Figure~\ref{fig:mechanism} sketches
the two rectification stages: the weight clamp (raw drift
net-negative, realized drift positive --- in both the synthetic and
the in-vivo regime, with the counter arithmetic reconciling exactly)
and the membrane's asymmetric climb barrier at the sticking point.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/mechanism.pdf}
\caption{The two rectifiers behind the \textsc{clamp-rectified}
mechanism. Left: counter waterfalls (thousands of cumulative
plasticity delta); raw $-$ absorbed $=$ applied holds exactly in both
regimes (asserted at render time from the recorded counters). Right:
the pinned sticking-point physics.}
\label{fig:mechanism}
\end{figure}
