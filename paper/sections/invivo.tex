...
attribution stays on validated ground. The single global gain $k$ was
\emph{derived} from the corpus RMS with a mid-band target of
$\approx450\,\mu$A; as implemented, $k$ multiplied milli-domain values,
so the realized pre-clamp current ran $\sim$1000$\times$ that target and
the rail, not the derived gain, set the effective drive (quantified in
the clamp caveat below). Deliberately rejected: an
...
railed on 89\% of its steps (H2) / 90\% (H1). A later distribution
probe --- which reproduces every banked run statistic before reporting
--- measured the exact pre-clamp currents: median
$3.1\times10^{5}\,\mu$A, with the railed fraction unchanged at every
ceiling the input path can carry (up to the i16 rail at
$\pm32{,}767\,\mu$A). The sign-dominance is therefore a construction
fact, not only a tails fact: the scaling constant, derived from
norm-unit RMS, was applied to milli-domain values, and the
\emph{effective} drive is sign-dominant, not amplitude-graded ---
``amplitude-informed'' is not what ran. The gates above pass on a \emph{shared} drive (both
...
protocol. The domain-corrected scaling --- the same gain applied in
norm units, measured at 2.7\% clamped with realized RMS exactly
$450.0\,\mu$A --- is the named fix, carried as a report-only arm in the
follow-on readout benchmark; per-dimension standardization remains the
deeper fix beyond it.
