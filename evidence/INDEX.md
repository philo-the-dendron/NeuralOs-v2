...
| `esp32c3-bringup/` | ESP32-C3 bring-up (2026-09-09) | **First spike on silicon.** `firmware/esp32c3` on a SuperMini (esp32c3 v0.4, native USB): burst 10,000 steps in 15,012 µs = 1,501 ns/step, 147 burst spikes = 14.7/s (exact host match), first loop spike step 56 (host 55, wall-time stamp explained), 295 spikes / 20.106 s sustained, blue LED. Blocking finding fixed on the branch: the image lacked the ESP-IDF app descriptor and the bootloader refused it (logged) | `README.md` (numbers + rebuild + capture), `bringup.log`, `boot-no-descriptor.log`, `factory-empty-flash.log`, `SHA256SUMS`; tool `tools/esp32c3_capture.py` |
