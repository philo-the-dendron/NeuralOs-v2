...
| `step5-calibration/` | Step-5 calibration burn (2026-09-05, ISA round-16) | **The lesion/graft positive control, verdict UNINFORMATIVE (PREREG §6 rule 6; the ruling is in ISA.md, cite that).** graft 0/3 vs LOCAL, 1/3 vs SCAT; lesion 0/4 vs SCAT, 0/4 vs MID. The readout benchmark's named first arm: no longer outstanding, the pre-registered bar not cleared, step 8 closed until a new instrument exists. | `PREREG.md` (frozen, stamps in §10), `measure.log`, `generate.log`, `arms.txt`, `seeds.txt`, `nulls.SHA256SUMS` (105), `SHA256SUMS`, `generator/`, `burn/` |
| `esp32c3-bringup/` | ESP32-C3 bring-up (2026-09-09) → round 36, capacity (2026-09-15) | **First spike on silicon, then the honest step cost.** `firmware/esp32c3` on a SuperMini (esp32c3 v0.4, native USB): one LIF neuron, then `FixedNetwork`, then a stranger's `.nir` by one command, then the capacity bar at both corners. The row is an index and holds no figure: numbers, `.text` pins, listings and rebuild lines live in the directory's README, one entry a round — first, bring-up (2026-09-09); second, alpha.6 (2026-09-10); third, round 27, the divisions (2026-09-12); fourth, round 30, the pin (2026-09-13); fifth, round 31, the recorder (2026-09-13); sixth, round 32, the network (2026-09-14); seventh, round 33, the version (2026-09-14); eighth, round 35, the base and the slot (2026-09-15); ninth, round 36, capacity (2026-09-15) — beside four host-bench sections and the mechanism read from the flashed ELFs. ROADMAP § 0.1.0 checks 2, 7 and 8 are measured here | `README.md` (numbers, mechanism, rebuild, capture, `.text` pins), `bringup.log`, `alpha6-remeasure.log`, `burst-loop-alpha{5,6}.dis`, `host-bench-alpha{5,6}.log`, `board-r27-alpha{6,7}.log`, `board-r30-{alpha7-before,pin}.log`, `host-bench-r30-2x2.log`, `board-r31-recorder.log`, `burst-loops-r31-recorder.dis`, `board-r32-network.log`, `board-r33-alpha8.log`, `host-bench-r33-alpha8.log`, `board-r35-{base,head,stranger}.log`, `board-r36-{head,neurons,dense}.log`, `burst-loops-r27-alpha{6,7}.dis`, `host-bench-r27-alpha{6,7}.log`, `host-bisect-r27.log`, `boot-no-descriptor.log`, `factory-empty-flash.log`, `SHA256SUMS`; tools `tools/esp32c3_capture.py`, `tools/esp32c3_trace_diff.py`; harness `proofs/spike-path-bench/` |
