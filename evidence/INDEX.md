...
| `qemu-riscv-gate/` | QEMU proof (2026-08-21) + round 33, the trace replay (2026-09-14) + round 34, the re-runs (2026-09-14) | **The riscv64gc no_std proof, both postures.** Leg A bare-metal none-elf: 175/175 cited checks, exit 0 (harness: `proofs/qemu-riscv-leg-a/`). Leg B user-mode musl: the REAL full suite on riscv64gc — 195/195 incl. the transmission trio + both Leg-C pins, exit 0, ~14 s. **Round 33 (alpha.8):** the twelve plasticity-off traces replayed bit for bit by `FixedNetwork` on bare-metal riscv64gc, diffed by the board's tool (harness: `proofs/qemu-trace-replay/`); ROADMAP § 0.1.0 check 1. **Round 34 (PR G):** Leg A at the alpha.8 stamp, 175/175, every check line identical to 2026-08-22's; the trace replay re-run, fourteen cases, 0 red, the two D8 witnesses among them, every row through the library's `neuralos_snn::fixed::row` | `README.md` (corrected pre-flight + rebuild), `leg-a.log`, `leg-a-alpha8.log`, `leg-b.log`, `replay-alpha8.log`, `replay-alpha9-g.log`, `SHA256SUMS` |
...
| `cold-start-orientation/` | Orientation arc (2026-08-28→29) | **The C1 bank.** The orientation layer's before/after cold-start measurement: 11→4 tool calls, 3→0 blind ledger seeks, 0→6 rules named unprompted; root cause AGENTS.md not auto-loaded. Frozen species-2 human record (raw transcripts session-local, deliberately not committed) | `README.md`, `SHA256SUMS` |
| `esp32c3-bringup/` | ESP32-C3 bring-up (2026-09-09) + the alpha.6 re-measure (2026-09-10) + round 27, the divisions (2026-09-12) + round 30, the pin (2026-09-13) + round 31, the recorder (2026-09-13) + round 32, the network (2026-09-14) + round 33, the version (2026-09-14) + round 35, the base and the slot (2026-09-15) + round 36, capacity (2026-09-15) | **First spike on silicon, then the honest step cost.** `firmware/esp32c3` on a SuperMini (esp32c3 v0.4, native USB): first loop spike step 56 (host 55, wall-time stamp explained), 147 burst spikes = 14.7/s (exact host match), 295 spikes / 20.1 s sustained, blue LED. Burst: 10,000 steps in 15,012 µs = 1,501 ns/step on the alpha.5 spine; **28,425 µs = 2,842 ns/step on alpha.6**, every behavior figure identical. Mechanism read from the two flashed ELFs (both pinned): alpha.5's burst loop kept the neuron in registers with one ROM 64-bit division per step, alpha.6's keeps it on the stack with two plus a `divu`; alpha.5's real-time loop already had the alpha.6 shape, so 1,501 was one scalarized loop and 2,842 the in-memory cost under the firmware's constant dt (true when written: a network's run-time dt cost more, 3,878 on alpha.6, round 27). Host bench: the `remove(0)` shift gone (forced arm 18.09 → 13.81 ns/step). Blocking finding fixed on the first branch: the image lacked the ESP-IDF app descriptor (logged). **Round 27 (alpha.7 in the tree):** two burst arms; the neuron as a network holds it (in memory, dt at run time) 3,878 → **1,579 ns/step**, the free loop 2,863 → 563, behavior bit-identical (147 spikes, first spike step 55, checksum `0b78b456`); the pinned loop has no ROM division left (both listings pinned); the host bench about 2 to 2.5 ns/step slower, bisected, recorded. **Round 30 (the pin, 1.98.1):** `.text` `0de3bd91…` → `33656de9…` with the compiler alone, the fold unchanged on the board (147 spikes, first spike step 55, checksum `0b78b456`), both burst arms +5.8 ns/step (pinned 1,585, free 569); a `.text` pin reproduces from its build path only (record-only); the host bench as a 2×2, tree × compiler: alpha.7's x86 cost stays on 1.98.1 (+2.5 forced, +2.6 control ns/step), the compiler moves each cell by less than 0.5. **Round 31 (the ring leaves the neuron):** `.text` `33656de9…` → `61639145…`, the fold unchanged on both arms (147 spikes, first spike step 55, checksum `0b78b456`), pinned 1,585 → 1,559 ns/step, free 569 → 570; no ring store left in either burst loop (listing pinned). **Round 32 (the network on the chip):** `FixedNetwork` on the board, `.text` `61639145…` → `b5351531…`: the frozen `feedforward-8` folds to the host pin, the twelve plasticity-off traces replay bit for bit, the neuron fold unchanged (§ Sixth entry). **Round 33 (the version, alpha.8):** the bump alone moves `.text` `b5351531…` → `6f8ec538…`, the fold unchanged on all three arms (147, 55, `0b78b456` on both neuron arms; 3,377, 5, `7e700ee1` on the network arm), the twelve replays identical (§ Seventh entry); the host bench against alpha.7 on 1.98.1: forced −0.6, control +0.05 ns/step (§ Host bench, round 33). **Round 35 (the slot, PR H):** PR G's `.text` `6f8ec538…` → `701490cf…` reproduced at `74712ee` and read on the board: the fourteen plasticity-off traces replay bit for bit, PR G's two D8 witnesses included, the fold unchanged on all three arms; at head, the slot empty, the same `.text` and the same fourteen; the one command, `stranger.sh run` on the snnTorch witness: the stranger's rows equal the host's trace, `15 cases, 0 red` (§ Eighth entry). **Round 36 (capacity, PR I):** the bar `44·N + 6·S ≤ 262,144` measured at its two corners on the board, one layer of 5,957 neurons and two layers of 201 with 40,401 synapses, each through `stranger.sh run`, `15 cases, 0 red`, the stack's high-water mark leaving 46,780 and 54,348 bytes of `.stack` free; the worst case the neuron corner, 5,850,980 ns/step mean, 10,946 µs a firing step; at head the mark and `report()`'s step count move `.text` `701490cf…` → `9e2e6ef1…`, the fold unchanged, `14 cases, 0 red` (§ Ninth entry) | `README.md` (numbers, mechanism, rebuild, capture, `.text` pins), `bringup.log`, `alpha6-remeasure.log`, `burst-loop-alpha{5,6}.dis`, `host-bench-alpha{5,6}.log`, `board-r27-alpha{6,7}.log`, `board-r30-{alpha7-before,pin}.log`, `host-bench-r30-2x2.log`, `board-r31-recorder.log`, `burst-loops-r31-recorder.dis`, `board-r32-network.log`, `board-r33-alpha8.log`, `host-bench-r33-alpha8.log`, `board-r35-{base,head,stranger}.log`, `board-r36-{head,neurons,dense}.log`, `burst-loops-r27-alpha{6,7}.dis`, `host-bench-r27-alpha{6,7}.log`, `host-bisect-r27.log`, `boot-no-descriptor.log`, `factory-empty-flash.log`, `SHA256SUMS`; tools `tools/esp32c3_capture.py`, `tools/esp32c3_trace_diff.py`; harness `proofs/spike-path-bench/` |
| `simd-hardening/` | SIMD hardening (2026-08-30→31) | **The rounding-fix cost.** `bench_simd` alternated before (`main@f82fc1e`) / after (branch tip), scalar column as control: vector path +12.5 % mean, +15.1 % min-to-min at N=1024 (1.79× → 1.60×), after slower in all six pairs; reviewer’s independent scalar-controlled run +15.3 %. Loaded box; a quiet-box number is still owed. Correctness numbers are not here: they are `#[ignore]` tests in `simd.rs` | `bench_alternated.log`, `README.md` (rebuild), `SHA256SUMS` |
