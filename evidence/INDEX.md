...
| `esp32c3-bringup/` | ESP32-C3 bring-up (2026-09-09) + the alpha.6 re-measure (2026-09-10) | **First spike on silicon, then the honest step cost.** `firmware/esp32c3` on a SuperMini (esp32c3 v0.4, native USB): first loop spike step 56 (host 55, wall-time stamp explained), 147 burst spikes = 14.7/s (exact host match), 295 spikes / 20.1 s sustained, blue LED. Burst: 10,000 steps in 15,012 µs = 1,501 ns/step on the alpha.5 spine; **28,425 µs = 2,842 ns/step on alpha.6**, every behavior figure identical. Mechanism read from the two flashed ELFs (both pinned): alpha.5's burst loop kept the neuron in registers with one ROM 64-bit division per step, alpha.6's keeps it on the stack with two plus a `divu`; alpha.5's real-time loop already had the alpha.6 shape, so 1,501 was one scalarized loop and 2,842 is the in-memory cost any network pays. Host bench: the `remove(0)` shift gone (forced arm 18.09 → 13.81 ns/step). Blocking finding fixed on the first branch: the image lacked the ESP-IDF app descriptor (logged) | `README.md` (numbers, mechanism, rebuild, capture, `.text` pins), `bringup.log`, `alpha6-remeasure.log`, `burst-loop-alpha{5,6}.dis`, `host-bench-alpha{5,6}.log`, `boot-no-descriptor.log`, `factory-empty-flash.log`, `SHA256SUMS`; tool `tools/esp32c3_capture.py`; harness `proofs/spike-path-bench/` |
